...
execute if score fanatic mwd_settings matches 0 run item replace entity @s enderchest.6 with pink_stained_glass{display:{Name:'{"text":"狂信者の人数設定 現在:0人","color":"light_purple","italic":false}'},change_page:1,fanatic:1}
execute if score fanatic mwd_settings matches 1 run item replace entity @s enderchest.6 with pink_stained_glass{display:{Name:'{"text":"狂信者の人数設定 現在:1人","color":"light_purple","italic":false}'},change_page:1,fanatic:1}
