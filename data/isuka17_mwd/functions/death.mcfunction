...
#　キルログ
execute if score #werewolf mwd_settings matches 1.. if entity @a[scores={mwd_killlog_killcount=1..}] run tellraw @a[team=ghost] [{"selector":"@a[scores={mwd_killlog_killcount=1..}]"},{"text":" → "},{"selector":"@a[scores={mwd_killlog_deathcount=1..}]"},{"text":" 死亡"}]
