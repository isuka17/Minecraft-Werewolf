...
# 当たった時の処理
execute at @e[type=armor_stand,tag=stanball,scores={fwd_settings=2}] run effect give @a[distance=..3.5,team=!mwd_ghost] blindness 5 0 true
execute at @e[type=armor_stand,tag=stanball,scores={fwd_settings=2}] if entity @a[distance=..3.5,team=!mwd_ghost] run particle explosion ~ ~1 ~ 0 0 0 0 1 force @a
execute at @e[type=armor_stand,tag=stanball,scores={fwd_settings=2}] if entity @a[distance=..3.5,team=!mwd_ghost] run particle end_rod ~ ~1 ~ 0 0 0 0.5 50 force @a
execute at @e[type=armor_stand,tag=stanball,scores={fwd_settings=2}] if entity @a[distance=..3.5,team=!mwd_ghost] run playsound entity.elder_guardian.curse master @a ~ ~ ~ 2 2
